...
class Oney extends Bnppirb
{
    ...
    protected $_code = \Lyranetwork\Bnppirb\Helper\Data::METHOD_ONEY;

    protected $_formBlockType = \Lyranetwork\Bnppirb\Block\Payment\Form\Oney::class;
    ...
    {
        $testMode = $this->bnppirbRequest->get('ctx_mode') == 'TEST';

        // override with FacilyPay Oney payment cards
        $this->bnppirbRequest->set('payment_cards', $testMode ? 'ONEY_SANDBOX' : 'ONEY');

        // set choosen option if any
        $info = $this->getInfoInstance();
        $option = @unserialize($info->getAdditionalInformation(\Lyranetwork\Bnppirb\Helper\Payment::ONEY_OPTION));
        if ($option && is_array($option)) {
            $this->bnppirbRequest->set('payment_option_code', $option['code']);
        }
    }
    ...
    public function assignData(\Magento\Framework\DataObject $data)
    {
        ...
        $option = $this->getOption($bnppirbData->getData('bnppirb_oney_option'));

        // init all payment data
        $info->setAdditionalInformation(\Lyranetwork\Bnppirb\Helper\Payment::ONEY_OPTION, serialize($option));
    ...
    public function isAvailable(\Magento\Quote\Api\Data\CartInterface $quote = null)
    {
        ...
        if (! preg_match(\Lyranetwork\Bnppirb\Helper\Checkout::CUST_ID_REGEX, $quote->getCustomerId())) {
            // customer id doesn't match FacilyPay Oney rules

            $msg = 'Customer ID "%s" does not match BNPP IRB specifications.';
            $msg .= ' The regular expression for this field is %s. FacilyPay Oney payment mean cannot be used.';
            $this->dataHelper->log(
                sprintf($msg, $quote->getCustomerId(), \Lyranetwork\Bnppirb\Helper\Checkout::CUST_ID_REGEX),
        ...
        if (! preg_match(\Lyranetwork\Bnppirb\Helper\Checkout::ORDER_ID_REGEX, $quote->getReservedOrderId())) {
            // order id doesn't match FacilyPay Oney rules

            $msg = 'The order ID "%s" does not match BNPP IRB specifications.';
            $msg .= 'The regular expression for this field is %s. FacilyPay Oney payment mean cannot be used.';
            $this->dataHelper->log(
                sprintf($msg, $quote->getReservedOrderId(), \Lyranetwork\Bnppirb\Helper\Checkout::ORDER_ID_REGEX),
                \Psr\Log\LogLevel::WARNING
            );
            return false;
        }
        ...
        foreach ($quote->getAllItems() as $item) {
            ...
            if (! preg_match(\Lyranetwork\Bnppirb\Helper\Checkout::PRODUCT_REF_REGEX, $item->getProductId())) {
                // product id doesn't match FacilyPay Oney rules

                $msg = 'Product reference "%s" does not match BNPP IRB specifications.';
                $msg .= 'The regular expression for this field is %s. FacilyPay Oney payment mean cannot be used.';
                $this->dataHelper->log(
                    sprintf($msg, $item->getProductId(), \Lyranetwork\Bnppirb\Helper\Checkout::PRODUCT_REF_REGEX),
                    \Psr\Log\LogLevel::WARNING
                );
                return false;
            }
