 ...
 * @package   bnppirb
 */
namespace Lyranetwork\Bnppirb\Model\Api;

if (! class_exists('BnppirbApi', false)) {
    ...
    class BnppirbApi
    {
        ...
        /**
         * Returns an array of languages accepted by the BNPP IRB payment platform.
         *
        ...
        /**
         * Return the list of currencies recognized by the BNPP IRB platform.
         *
         * @return array[int][BnppirbCurrency]
         */
        public static function getSupportedCurrencies()
        {
            $currencies = array(
                ...
                array('FRF', '250', 2), array('GNF', '324', 0), array('HKD', '344', 2), array('HUF', '348', 2),
                array('INR', '356', 2), array('IDR', '360', 0), array('JPY', '392', 0), array('KRW', '410', 0),
                array('KWD', '414', 3), array('LVL', '428', 2), array('LTL', '440', 2), array('MYR', '458', 2),
                array('MXN', '484', 2), array('MAD', '504', 2), array('NZD', '554', 2), array('NOK', '578', 2),
                array('PHP', '608', 2), array('RUB', '643', 2), array('SGD', '702', 2), array('ZAR', '710', 2),
                array('SEK', '752', 2), array('CHF', '756', 2), array('THB', '764', 2), array('TND', '788', 3),
                array('GBP', '826', 2), array('USD', '840', 2), array('TWD', '901', 2), array('RON', '946', 2),
                array('TRY', '949', 2), array('XOF', '952', 0), array('XPF', '953', 0), array('BGN', '975', 2),
                array('EUR', '978', 2), array('PLN', '985', 2), array('BRL', '986', 2)
            );

            $bnppirb_currencies = array();

            foreach ($currencies as $currency) {
                $bnppirb_currencies[] = new BnppirbCurrency($currency[0], $currency[1], $currency[2]);
            }

            return $bnppirb_currencies;
        }
        ...
        public static function getCurrencyNumCode($alpha3)
        {
            ...
            return ($currency instanceof BnppirbCurrency) ? $currency->getNum() : null;
        }

        /**
         * Returns an array of card types accepted by the BNPP IRB payment platform.
         *
         * @return array[string][string]
         */
        ...
        {
            return array(
                'VISA' => 'Visa', 'VISA_ELECTRON' => 'Visa Electron', 'MAESTRO' => 'Maestro', 'MASTERCARD' => 'MasterCard'
            );
        }

        /**
         * Compute a BNPP IRB signature. Parameters must be in UTF-8.
         *
