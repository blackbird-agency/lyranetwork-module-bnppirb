 ...
 * @package   bnppirb
 */
namespace Lyranetwork\Bnppirb\Model;

class BnppirbConfigProvider implements \Magento\Checkout\Model\ConfigProviderInterface
{
    ...
    /**
     *
     * @var \Lyranetwork\Bnppirb\Model\Method\Bnppirb
     */
    protected $method;
    ...
    {
        return $this->urlBuilder->getUrl('bnppirb/payment/redirect', [
            '_secure' => true
        ]);
    }
    ...
    private function getAvailableCcTypes()
    {
        ...
        foreach ($this->method->getAvailableCcTypes() as $value => $label) {
            $asset = $this->assetRepo->createAsset('Lyranetwork_Bnppirb::images/cc/' . strtolower($value) . '.png');

            $icon = false;
            if ($this->dataHelper->isPublishFileImageExists($asset->getRelativeSourceFilePath())) {
                $icon = $this->getViewFileUrl('Lyranetwork_Bnppirb::images/cc/' . strtolower($value) . '.png');
            }
