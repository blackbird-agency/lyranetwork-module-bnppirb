...
class UpdateMultiPaymentObserver implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        ...
        if ($payment->getMethod() != 'bnppirb_multi') {
            // not bnppirb multiple payment, do nothing
            return $this;
        }

        // retreive selected option
        $option = @unserialize($payment->getAdditionalInformation(\Lyranetwork\Bnppirb\Helper\Payment::MULTI_OPTION));
        if (is_array($option) && ! empty($option)) {
            $payment->setMethod('bnppirb_multi_' . $option['count'] . 'x');
        }
