...
/**
 * Custom renderer for the BNPP IRB category mapping field.
 */
class CategoryMapping extends \Lyranetwork\Bnppirb\Block\Adminhtml\System\Config\Form\Field\FieldArray\ConfigFieldArray
{

    /**
     *
     * @var \Lyranetwork\Bnppirb\Model\System\Config\Source\CategoryFactory
     */
    protected $bnppirbCategoryFactory;
     ...
     * @param \Lyranetwork\Bnppirb\Model\System\Config\Source\CategoryFactory $bnppirbCategoryFactory
     * @param \Magento\Catalog\Model\CategoryFactory $categoryFactory
     * @param array $data
     */
    public function __construct(
        \Magento\Backend\Block\Template\Context $context,
        \Lyranetwork\Bnppirb\Model\System\Config\Source\CategoryFactory $bnppirbCategoryFactory,
        \Magento\Catalog\Model\CategoryFactory $categoryFactory,
        array $data = []
    ) {
        $this->bnppirbCategoryFactory = $bnppirbCategoryFactory;
    ...
    public function _prepareToRender()
    {
        ...
        $options = [];
        foreach ($this->bnppirbCategoryFactory->create()->toOptionArray(true) as $option) {
        ...
        $this->addColumn(
            'bnppirb_category',
            [
                'label' => __('BNPP IRB category'),
                'style' => 'width: 200px;',
                'renderer' => $this->getListRenderer('_bnppirbCategory', $options)
            ]
        );
        ...
        if ($allCategories && is_array($allCategories) && ! empty($allCategories)) {
            foreach ($allCategories as $code => $name) {
                $value[uniqid('_' . $code . '_')] = [
                    ...
                    'bnppirb_category' => 'FOOD_AND_GROCERY',
                    'mark' => true
                ];
            }
        }
