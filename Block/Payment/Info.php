...
use Lyranetwork\Bnppirb\Model\Api\BnppirbResponse;
...
class Info extends \Magento\Payment\Block\Info
{
     ...
     * @var string
     */
    protected $_template = 'Lyranetwork_Bnppirb::payment/info.phtml';
    ...
    public function getResultDescHtml()
    {
        $allResults = @unserialize(
            $this->getInfo()->getAdditionalInformation(\Lyranetwork\Bnppirb\Helper\Payment::ALL_RESULTS)
        );
        ...
        if (! is_array($allResults) || empty($allResults)) {
            ...
            foreach ($keys as $key) {
                ...
                if ($key === 'result' && $allResults[$key] == '30') { // append form error if any
                    $label .= ' ' . BnppirbResponse::extraMessage($allResults['extra_result']);
                }
    ...
    public function getPaymentDetailsHtml($backend = true)
    {
        ...
        if ($backend) {
            $userChoice = $payment->getAdditionalInformation(\Lyranetwork\Bnppirb\Helper\Payment::BRAND_USER_CHOICE);
            if ($userChoice === true) {
    ...
    public function translate($code, $type, $appendCode = false)
    {
        ...
        return BnppirbResponse::translate($code, $type, $lang, $appendCode);
    }
}
