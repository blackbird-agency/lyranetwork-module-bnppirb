 ...
 * @package   bnppirb
 */
namespace Lyranetwork\Bnppirb\Helper;
...
class Checkout
{
    ...
    public function checkCustormers($scope, $scopeId)
    {
        ...
        foreach ($collection as $customer) {
            if (! preg_match(self::CUST_ID_REGEX, $customer->getId())) {
                // a customer id doesn't match BNPP IRB rules

                $msg = '';
                $msg .= __(
                    'Customer ID &laquo;%1&raquo; does not match BNPP IRB specifications.',
                    $customer->getId()
                )->render() . ' ';
    ...
    private function checkOrderId($orderId)
    {
        if (! preg_match(self::ORDER_ID_REGEX, $orderId)) {
            // the potential next order id doesn't match BNPP IRB rules

            $msg = '';
            $msg .= __(
                'The next order ID  &laquo;%1&raquo; does not match BNPP IRB specifications.',
                $orderId
            )->render() . ' ';
    ...
    public function checkProducts($scope, $scopeId)
    {
        ...
        foreach ($collection as $product) {
            if (! preg_match(self::PRODUCT_REF_REGEX, $product->getId())) {
                // product id doesn't match BNPP IRB rules

                $msg = '';
                $msg .= __(
                    'Product reference &laquo;%1&raquo; does not match BNPP IRB specifications.',
                    $product->getId()
                )->render() . ' ';
    ...
    public function toBnppirbCategory($categoryIds)
    {
        ...
        if (is_array($categoryMapping) && ! empty($categoryMapping) && is_array($categoryIds) && ! empty($categoryIds)) {
            foreach ($categoryMapping as $id => $category) {
                if (in_array($category['code'], $categoryIds)) {
                    return $category['bnppirb_category'];
                }
            }
        }
    ...
    public function setCartData($order, &$bnppirbRequest)
    {
        $notAllowed = '#[^A-Z0-9ÁÀÂÄÉÈÊËÍÌÎÏÓÒÔÖÚÙÛÜÇ ]#ui';

        // used currency
        $currency = BnppirbApi::findCurrencyByNumCode($bnppirbRequest->get('currency'));
        ...
        foreach ($order->getAllItems() as $item) {
            ...
            if (! $item->getParentItem()) {
                ...
                $bnppirbRequest->addProduct(
                    preg_replace($notAllowed, ' ', $label),
                    $priceInCents,
                    $qty,
                    $item->getProductId(),
                    $this->toBnppirbCategory($product->getCategoryIds())
                );
        ...
        $bnppirbRequest->set('insurance_amount', 0); // by default, shipping insurance amount is not available in Magento
        $bnppirbRequest->set('shipping_amount', $currency->convertAmountToInteger($order->getShippingAmount()));

        // recalculate tax_amount to avoid rounding problems
        $taxAmount = $bnppirbRequest->get('amount') - $subtotal - $bnppirbRequest->get('shipping_amount') -
             $bnppirbRequest->get('insurance_amount');
        if ($taxAmount <= 0) { // when order is discounted
            $taxAmount = $currency->convertAmountToInteger($order->getTaxAmount());
        }

        $bnppirbRequest->set('tax_amount', $taxAmount);
    }

    public function setOneyData($order, &$bnppirbRequest)
    {
        // by default, clients are private
        $bnppirbRequest->set('cust_status', 'PRIVATE');
        $bnppirbRequest->set('ship_to_status', 'PRIVATE');
        ...
        if ($order->getIsVirtual() || ! $order->getShippingMethod()) {
            ...
            $bnppirbRequest->set('ship_to_type', 'ETICKET');
            $bnppirbRequest->set('ship_to_speed', 'EXPRESS');
        } else {
            ...
            switch ($shippingMethod['type']) {
                ...
                case 'RECLAIM_IN_STATION':
                    ...
                    $bnppirbRequest->set('ship_to_street', $address);
                    $bnppirbRequest->set('ship_to_zip', $order->getShippingAddress()->getPostcode());
                    $bnppirbRequest->set('ship_to_city', $order->getShippingAddress()->getCity());
                    $bnppirbRequest->set('ship_to_country', 'FR');
                    $bnppirbRequest->set('ship_to_street2', null); // not sent to FacilyPay Oney
                    $bnppirbRequest->set('ship_to_state', null);
                    $bnppirbRequest->set('ship_to_phone_num', null);
                    ...
                    $bnppirbRequest->set('ship_to_delivery_company_name', $method);
                    break;
                default:
                    $address = '';
                    $address .= $order->getShippingAddress()->getStreet(1);
                    ...
                    $bnppirbRequest->set('ship_to_delivery_company_name', $shippingMethod['oney_label']);
                    break;
            }

            $bnppirbRequest->set('ship_to_type', $shippingMethod['type']);
            $bnppirbRequest->set('ship_to_speed', $shippingMethod['speed']);
        }
    }
