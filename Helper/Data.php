...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    const METHOD_STANDARD = 'bnppirb_standard';

    const METHOD_MULTI = 'bnppirb_multi';

    const METHOD_CHOOZEO = 'bnppirb_choozeo';

    const METHOD_GIFT = 'bnppirb_gift';

    const METHOD_ONEY = 'bnppirb_oney';

    const METHOD_PAYPAL = 'bnppirb_paypal';

    const METHOD_SOFORT = 'bnppirb_sofort';

    const METHOD_POSTFINANCE = 'bnppirb_postfinance';
    ...
    /**
     *
     * @var \Lyranetwork\Bnppirb\Model\Logger\Bnppirb
     */
    protected $logger;
    ...
    /**
     *
     * @param \Lyranetwork\Bnppirb\Helper\Context $context
     ...
     * @param \Lyranetwork\Bnppirb\Model\Logger\Bnppirb
     * @param \Magento\Framework\App\State $appState
     * @param \Zend\Http\PhpEnvironment\RemoteAddress $remoteAddress
     * @param \Magento\Framework\Filesystem\Io\File $file
     */
    public function __construct(
        \Lyranetwork\Bnppirb\Helper\Context $context,
        ...
        \Lyranetwork\Bnppirb\Model\Logger\Bnppirb $logger,
    ...
    /**
     * Shortcut method to get general BNPP IRB module configuration.
     *
     * @param string $field
    ...
    public function getCommonConfigData($field, $storeId = null)
    {
        ...
        return $this->scopeConfig->getValue('bnppirb/general/' . $field, ScopeInterface::SCOPE_STORE, $storeId);
    }
    ...
    public function getReturnUrl($storeId = null)
    {
        ...
        return $this->_getUrl('bnppirb/payment/response', $params);
    }
    ...
    {
        $filePath = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath('bnppirb/images/' . $fileName);
    ...
    {
        // $path is as payment_[lang]/bnppirb/bnppirb_[group]/bnppirb_[sub_group]
        $parts = explode('/', $path);
    ...
    public function updateMultiPaymentModelConfig($options)
    {
        foreach ($options as $option) {
            $this->resourceConfig->saveConfig(
                'payment/bnppirb_multi_' . $option['count'] . 'x/model',
                \Lyranetwork\Bnppirb\Model\Method\Multix::class,
    ...
    public function getMultiPaymentModelConfig()
    {
        ...
        $select = $connection->select()
            ...
            ->where('path LIKE ?', 'payment/bnppirb\_multi\_%x/model');
     ...
     *
     * @throws \Lyranetwork\Bnppirb\Model\WsException
     */
    public function checkWsRequirements()
    {
        if (! extension_loaded('soap')) {
            throw new \Lyranetwork\Bnppirb\Model\WsException(
                'SOAP extension for PHP must be enabled on the server in order to use BNPP IRB web services.'
            );
        }

        if (! extension_loaded('openssl')) {
            throw new \Lyranetwork\Bnppirb\Model\WsException(
                'OPENSSL extension for PHP must be enabled on the server in order to use BNPP IRB web services.'
            );
        }
    }
