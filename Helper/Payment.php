...
class Payment
{
    ...
    const IDENTIFIER = 'bnppirb_identifier';

    // key to save choosen multi option
    const MULTI_OPTION = 'bnppirb_multi_option';

    // key to save choosen Choozeo option
    const CHOOZEO_OPTION = 'bnppirb_choozeo_option';

    // key to save choosen Oney option
    const ONEY_OPTION = 'bnppirb_oney_option';

    // key to save risk control results
    const RISK_CONTROL = 'bnppirb_risk_control';

    // key to save risk assessment results
    const RISK_ASSESSMENT = 'bnppirb_risk_assessment';

    // key to save risk assessment results
    const ALL_RESULTS = 'bnppirb_all_results';

    const TRANS_UUID = 'bnppirb_trans_uuid';

    const BRAND_USER_CHOICE = 'bnppirb_brand_user_choice';
     ...
     * @param \Lyranetwork\Bnppirb\Model\Api\BnppirbResponse $response
     */
    public function registerOrder(
        \Magento\Sales\Model\Order $order,
        \Lyranetwork\Bnppirb\Model\Api\BnppirbResponse $response
    ) {
        ...
        // try to save BNPP IRB identifier if any
    ...
    /**
     * Get new order state and status according to BNPP IRB response.
     *
     * @param \Magento\Sales\Model\Order $order
     * @param \Lyranetwork\Bnppirb\Model\Api\BnppirbResponse $response
    ...
    public function updatePaymentInfo(\Magento\Sales\Model\Order $order, \Lyranetwork\Bnppirb\Model\Api\BnppirbResponse $response)
    {
        ...
        $order->getPayment()
            ...
            ->setAdditionalInformation(\Lyranetwork\Bnppirb\Helper\Payment::ALL_RESULTS, serialize($response->getAllResults()))
            ->setAdditionalInformation(\Lyranetwork\Bnppirb\Helper\Payment::TRANS_UUID, $response->get('trans_uuid'));
        ...
        if ($response->get('brand_management')) {
            ...
            $order->getPayment()->setAdditionalInformation(\Lyranetwork\Bnppirb\Helper\Payment::BRAND_USER_CHOICE, $userChoice);
        }
        ...
        if ($response->get('card_brand') == 'MULTI') { // multi brand
            ...
            $currency = BnppirbApi::findCurrencyByNumCode($response->get('currency'));
            ...
            $option = @unserialize($order->getPayment()->getAdditionalInformation(\Lyranetwork\Bnppirb\Helper\Payment::MULTI_OPTION));

            if ($response->get('sequence_number') == 1 && (stripos($order->getPayment()->getMethod(), 'bnppirb_multi') === 0)
                ...
                for ($i = 1; $i <= $count; $i++) {
                    ...
                    if (($rate = $response->get('change_rate')) && $response->get('effective_currency') &&
                        ...
                        // effective amount
                        $effectiveCurrency = BnppirbApi::findCurrencyByNumCode($response->get('effective_currency'));
    ...
    public function saveIdentifier(
        ...
        if ($response->get('identifier') && (
            ...
            $customer->setData('bnppirb_identifier', $response->get('identifier'));
            $customer->save();
    ...
    /**
     * Convert BNPP IRB transaction status to magento transaction type.
     *
     * @param string $bnppirbType
     * @return string
     */
    public function convertTransactionType($bnppirbType)
    {
        $type = false;
