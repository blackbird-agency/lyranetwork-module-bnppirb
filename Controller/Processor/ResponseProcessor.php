...
class ResponseProcessor
{
    ...
    public function execute(\Lyranetwork\Bnppirb\Api\ResponseActionInterface $controller)
    {
        ...
        if (! $bnppirbResponse->isAuthentified()) {
            ...
            $this->dataHelper->log(
                "{$this->dataHelper->getIpAddress()} tries to access bnppirb/payment/response page without valid signature with parameters: " . json_encode($request),
        ...
        if (! $orderId) {
            $this->dataHelper->log(
                "Order ID not returned. Payment result : " . $bnppirbResponse->getLogMessage(),
                \Psr\Log\LogLevel::ERROR
            );
            return $controller->redirectError($order);
        }
        ...
        if ($order->getStatus() == 'pending_payment') {
            ...
            'bnppirb_to_validate' /* payment will be done after manual validation */
            ];
