 ...
 * @package   bnppirb
 */
namespace Lyranetwork\Bnppirb\Controller\Payment;

class Response extends \Magento\Framework\App\Action\Action implements \Lyranetwork\Bnppirb\Api\ResponseActionInterface
{
    ...
    /**
     *
     * @var \Lyranetwork\Bnppirb\Controller\Processor\ResponseProcessor
     */
     ...
     * @param \Lyranetwork\Bnppirb\Controller\Processor\ResponseProcessor $responseProcessor
    ...
    public function __construct(
        ...
        \Lyranetwork\Bnppirb\Controller\Processor\ResponseProcessor $responseProcessor,
    ...
    public function redirectResponse($order, $success, $checkUrlWarn = false)
    {
        ...
        if ($features['prodfaq'] && ($this->dataHelper->getCommonConfigData('ctx_mode', $storeId) == 'TEST')) {
            ...
            $message .= '<a href="https://e-paiement-securite-bici.com/html/faq/prod" target="_blank">https://e-paiement-securite-bici.com/html/faq/prod</a>';
            ...
            if ($checkUrlWarn) {
                ...
                if ($this->dataHelper->isMaintenanceMode()) {
                    ...
                    $message = __('The automatic validation has not worked. Have you correctly set up the notification URL in your BNPP IRB Back Office ?');
                    $message .= '<br /><br />';
    ...
    private function createResult($path, $params)
    {
        if ($this->getRequest()->getParam('iframe', false)) {
            ...
            $block = $result->getLayout()
                ->createBlock(\Lyranetwork\Bnppirb\Block\Payment\Iframe\Response::class)
                ->setTemplate('Lyranetwork_Bnppirb::payment/iframe/response.phtml')
